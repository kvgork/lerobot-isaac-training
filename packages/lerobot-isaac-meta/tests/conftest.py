...
1. In monorepo mode: add sibling package src directories to sys.path so tests
   can import lerobot_isaac_adapters and other siblings without requiring
   pixi install.
...
# Sibling packages that must exist alongside meta for "in monorepo" to be true.
_REQUIRED_SIBLINGS = (
