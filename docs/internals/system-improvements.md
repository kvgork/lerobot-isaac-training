...
### 2026-06-20 — curriculum_controller cannot drive the carry-place distance curriculum (3 incompatible "stage" abstractions)
- **Type:** systemic (Phase-2 autonomy design gap)
- **Discovered in:** `20260620-084313-continue-level3-plan` (Phase-2 launch vet workflow, curriculum-align dimension)
- **Gap:** Three orthogonal "stage" notions exist: (1) the shipped `curriculum_controller` advances an integer stage **2-4 = DR-intensity** via `LEROBOT_ISAAC_STAGE`; (2) the 2026-06-16 plan's fine curriculum advances die **SPAWN DISTANCE** via `LEROBOT_ISAAC_OBJECT_X/Y` over 5 steps (6→18cm); (3) `lerobot-curriculum-agent` models 1-6 per-dataset. The controller's `stage_env_value` emits `LEROBOT_ISAAC_STAGE` only and never `OBJECT_X/Y`, so it **cannot drive the distance ladder**; its thresholds (0.85/0.80/0.75) also contradict the plan's ≥50%-over-trailing-60-episodes rule, and it has **zero production callers**. Net: Phase-2 multi-stage advancement is currently MANUAL (relaunch per stage with new `OBJECT_X/Y` + `checkpoint.resume_from`).
- **Suggested fix:** re-model `curriculum_controller` as a STEP index 0-4 over an `(OBJECT_X, OBJECT_Y)` distance table that emits `OBJECT_X/Y` (not `LEROBOT_ISAAC_STAGE`), set thresholds to 0.50, widen the stage range to 5 steps/4 advances, and add a thin driver (in `_run_wm_isaac_overnight.sh` or a new launcher) that reads trailing-60-episode place-success + chains `checkpoint.resume_from` between stages. Until then, Phase-2 stays manual.
- **Update 2026-06-20 (partial fix shipped):** the distance-curriculum CAPABILITY landed (`curriculum_controller` commit `1b82150`: `DISTANCE_LADDER`, `decide_advance_distance`, `distance_step_env_values`, `advance_distance` → separate `curriculum_distance.json`; 51 tests). The auto-relaunch DRIVER is still pending, and is **blocked on a clean place-success signal**: (a) `_sim_eval.py` evals **lerobot policies only** — it cannot eval a DreamerV3 sheeprl checkpoint, so it can't supply `task_success` for the WM campaign; (b) DreamerV3's TB logs only `Rewards/rew_avg` + `Game/ep_len_avg` + `Time/sps_env_interaction` — **no place/task-success tag**. The driver's advance signal must therefore be DERIVED (place ⇒ early `place_termination` ⇒ `Game/ep_len_avg` drops below `max_episode_steps=300` + `rew_avg` rises), and that derivation must be calibrated against real post-warmup data before it can safely gate auto-advance. Build the driver once stage-1 (`cp-stage1-r8-20260620`) produces post-warmup checkpoints + a readable place-success curve. Cleaner long-term fix: log a `Game/place_success` scalar from the env's termination-manager success-count in the sheeprl plugin.

### 2026-06-22 — le_world_model subprocess tests stale (backend → _lewm_minimal)
- **Type:** test debt (pre-existing, NOT a regression)
- **Discovered in:** P2E MVP verification (`20260620-084313-continue-level3-plan`). `test_targets_subprocess.py::TestWmLeWorldModelSubprocess` (test_train_world_model_command_built / test_nonzero_returncode_propagated / test_pred_loss_re_emitted) FAIL at HEAD AND at the parent commit — the le_world_model backend switched to `_lewm_minimal` (no `train_world_model`), but the tests still assert the old `train_world_model` command. Confirmed pre-existing via git-checkout of the parent.
- **Fix:** update the TestWmLeWorldModelSubprocess expectations to the `_lewm_minimal` command shape (or mark the backend's CLI as intentionally changed). Low priority (le_world_model is BLOCKED upstream per CLAUDE.md; dreamerv3 is the active WM path).
