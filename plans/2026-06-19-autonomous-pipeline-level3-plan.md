...
- **FIXED — `_sim_eval.py` `task_success` spawn-pose bug.** The verifier read the object pose *after* `env.step()`, but Isaac `ManagerBasedRLEnv.step()` auto-resets terminated envs internally → it was measuring the next episode's spawn pose (≈0 regardless of policy). Now sources `task_success` from the env's `success` termination-term verdict (cached pre-reset; for pick_and_place that term IS `place_termination`→`object_in_bin`, so still the canonical predicate). New helper `_read_success_term(env)` + 9 CPU tests. **[P0] GPU-VERIFIED 2026-06-20** — sim eval (ACT on `so101-sim-pickplace-demos-tuned`, 3 ep) ran clean; `_read_success_term` read the real Isaac `termination_manager` (non-None, no fallback — output `success_criterion` shows the full term-verdict path), task_success tracked pc_success (0/3, all timeouts: the policy never places, so 0 is legit; a positive `task_success=1` still awaits a placing policy). **Isaac env boot is HEALTHY** (booted + ran 3 eps), so the cur2b abort (4 days stale) was likely dreamer-path-specific, not an env blocker — de-risks the Phase-2 curriculum run.
