...
def _write_meta(
    workspace_root: Path, repo_id: str, episodes: list[dict], tasks: list[dict]
) -> Path:
...
def test_synthetic_task_join(workspace_root):
    _write_meta(
        ...
        "merged_v2",
        episodes=[
            {"episode_index": 0, "length": 30, "source": "real", "tasks_index": 0}
        ],
...
def test_synthetic_missing_tasks_index(workspace_root):
    ...
    meta_dir.mkdir(parents=True, exist_ok=True)
    pd.DataFrame([{"episode_index": 0, "length": 10, "source": "real"}]).to_parquet(
        meta_dir / "episodes.parquet", index=False
    )
    pd.DataFrame([{"tasks_index": 0, "task": "pick_and_place"}]).to_parquet(
